...
const KEYWORD_RULES = [
    ...
    {
        pattern: /\bjwt\b|\boauth\b|\brbac\b|\babac\b|\bsaml\b|\boidc\b|\bmtls\b/i,
        category: 'Architecture & Security',
    },
    {
        pattern:
            /microservice|hexagonal|clean[\s.-]?architecture|domain[\s.-]?driven|layered[\s.-]?architecture|event[\s.-]?driven|service[\s.-]?oriented|soa\b/i,
        category: 'Architecture & Security',
    },
    ...
    {
        pattern: /docker|kubernetes|terraform|jenkins|gitlab\s*ci|github\s*action|argocd|helm/i,
    ...
    {
        pattern: /data\s*structure|algorithm|\boop\b|object[\s-]?oriented|\bsolid\b|design\s*pattern/i,
        category: 'Concepts',
    },
...
export function normalizeSkills(skills) {
    ...
    if (isLegacyMegaBucket) {
        ...
        categories.length = 0;
        try {
            localStorage.setItem(SPLIT_LEGACY_FLAG_KEY, '1');
        } catch {
            /* private mode */
        }
    }
