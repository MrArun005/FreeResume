...
import {
    ChevronRight, ChevronLeft, ZoomIn, ZoomOut, Monitor, Smartphone,
    CheckCircle, Palette, Layout as LayoutIcon
} from 'lucide-react';
        ...
        <div className={`${mobileView === 'preview' ? 'flex' : 'hidden'} lg:flex flex-1 bg-gray-200 overflow-auto h-full relative print-area flex-col items-center p-4 lg:p-12 gap-8 pb-24 lg:pb-12 transition-all duration-300 ${isSidebarOpen ? 'xl:pr-[340px]' : 'xl:pr-12'}`}>
            ...
            <div className="fixed top-24 right-6 flex flex-col gap-3 z-30 no-print">
                <div className="bg-slate-900/60 backdrop-blur-xl p-2 rounded-2xl shadow-[0_8px_32px_rgba(0,0,0,0.2)] border border-white/10 flex flex-col gap-2">
                    ...
                    <button onClick={() => setScale(s => Math.min(s + 0.1, 2))} className="p-2.5 hover:bg-white/10 rounded-xl text-gray-300 transition-all duration-300" title="Zoom In">
                        <ZoomIn size={20} />
                    </button>
                    <button onClick={() => setScale(s => Math.max(s - 0.1, 0.5))} className="p-2.5 hover:bg-white/10 rounded-xl text-gray-300 transition-all duration-300" title="Zoom Out">
                        <ZoomOut size={20} />
                    </button>
                </div>
            </div>
            ...
            <div className={`fixed right-0 top-[64px] bottom-0 w-[340px] bg-slate-950/80 backdrop-blur-2xl border-l border-white/5 shadow-[-10px_0_30px_rgba(0,0,0,0.2)] transform transition-transform duration-500 cubic-bezier(0.4, 0, 0.2, 1) z-20 overflow-y-auto no-print ${isSidebarOpen ? 'translate-x-0' : 'translate-x-full'}`}>
