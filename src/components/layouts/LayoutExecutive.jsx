            ...
            <div className="px-16 py-12 h-full flex flex-col">
                ...
                {(!pageIndex || pageIndex === 0) && (
                    <header className="text-center mb-10 border-b border-slate-200 pb-8">
                        <h1 className="text-4xl font-bold text-slate-900 mb-3 tracking-wide uppercase" style={{ fontFamily: 'Playfair Display, serif' }}>
                            {personal.fullName}
                        </h1>
                        ...
                        <div className="flex flex-wrap justify-center gap-6 text-sm text-slate-500 font-sans">
                            {personal.email && (
                                <div className="flex items-center gap-2">
                                    <span className="w-1 h-1 bg-[#D4AF37] rounded-full"></span> {personal.email}
                                </div>
                            )}
                            {personal.phone && (
                                <div className="flex items-center gap-2">
                                    <span className="w-1 h-1 bg-[#D4AF37] rounded-full"></span> {personal.phone}
                                </div>
                            )}
                            {personal.location && (
                                <div className="flex items-center gap-2">
                                    <span className="w-1 h-1 bg-[#D4AF37] rounded-full"></span> {personal.location}
                                </div>
                            )}
                            {personal.socials?.map((social, index) => (
                                <a key={index} href={social.url} className="flex items-center gap-2 hover:text-[#D4AF37] transition-colors">
                                    <span className="w-1 h-1 bg-[#D4AF37] rounded-full"></span> {social.network}
                                </a>
                            ))}
                        </div>
                    </header>
                        ...
                        <section id="section-experience">
                            {shouldRenderTitle('experience') && (
                                <div id="section-title-experience" className="flex items-center gap-4 mb-6">
                                    ...
                                    <h3 className="text-md font-bold uppercase tracking-widest text-slate-900 font-sans">Professional Experience</h3>
                                    <div className="h-px bg-slate-300 flex-1"></div>
                                </div>
                            )}
                    ...
                    {education.length > 0 && (
                        <section id="section-education">
                            {shouldRenderTitle('education') && (
                                <div id="section-title-education" className="flex items-center gap-4 mb-6 mt-2">
                                    <div className="h-px bg-slate-300 flex-1"></div>
                                    <h3 className="text-md font-bold uppercase tracking-widest text-slate-900 font-sans">Education</h3>
                                    <div className="h-px bg-slate-300 flex-1"></div>
                                </div>
                            )}
                            <div className="grid grid-cols-1 gap-4">
                                {education.map((edu) => (
                                    <div key={edu.id} id={`item-${edu.id}`} className="flex justify-between items-end border-b border-slate-100 pb-2">
                                        <div>
                                            <h4 className="font-bold text-slate-900 text-lg">{edu.school}</h4>
                                            <div className="text-slate-600 italic">{edu.degree}</div>
                                        </div>
                                        <span className="text-sm text-slate-500 font-sans font-medium">{edu.date}</span>
                                    </div>
                                ))}
                            </div>
                        </section>
                        ...
                        <section id="section-skills">
                            {shouldRenderTitle('skills') && (
                                <div id="section-title-skills" className="flex items-center gap-4 mb-6 mt-2">
                                    <div className="h-px bg-slate-300 flex-1"></div>
                                    <h3 className="text-md font-bold uppercase tracking-widest text-slate-900 font-sans">Core Competencies</h3>
                                    <div className="h-px bg-slate-300 flex-1"></div>
                                </div>
                            )}
                        ...
                        section.items.length > 0 && (
                            <section key={section.id} id={`section-${section.id}`}>
                                {shouldRenderTitle(section.id) && (
                                    <div id={`section-title-${section.id}`} className="flex items-center gap-4 mb-6 mt-2">
                                        <div className="h-px bg-slate-300 flex-1"></div>
                                        <h3 className="text-md font-bold uppercase tracking-widest text-slate-900 font-sans">{section.title}</h3>
                                        <div className="h-px bg-slate-300 flex-1"></div>
                                    </div>
                                )}
                                <div className="space-y-6">
                                    {section.items.map((item) => (
                                        <div key={item.id} id={`item-${item.id}`}>
                                            ...
                                            {item.subtitle && <div className="text-[#D4AF37] text-xs font-bold uppercase mb-2">{item.subtitle}</div>}
                                            {item.description && <p className="text-slate-700 text-sm leading-relaxed whitespace-pre-line text-justify">{item.description}</p>}
                                        </div>
                                    ))}
                                </div>
                            </section>
