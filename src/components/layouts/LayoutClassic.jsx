...
const LayoutClassic = ({ data, theme, pageIndex, isMeasurement }) => {
    return (
        <div className={`p-10 ${isMeasurement ? 'h-auto overflow-visible' : 'h-[297mm] overflow-hidden'} font-sans text-gray-800 relative box-border`}>
            {/* Header - Only on first page */}
            {pageIndex === 0 && (
                <div id="section-personal" className={`border-b-2 ${theme.border} pb-6 mb-6 flex justify-between items-start`}>
                    <div>
                        ...
                        {data.personal.email && (
                            <a href={`mailto:${data.personal.email}`} className="flex items-center gap-2 text-gray-800 hover:text-blue-600 hover:underline print:text-blue-600 print:underline">
                    ...
                    <div className="flex flex-col items-end gap-1 text-sm">
                        {data.personal.phone && (
                            <a href={`tel:${data.personal.phone}`} className="flex items-center gap-2 text-gray-800 hover:text-blue-600 hover:underline print:text-blue-600 print:underline">
                                {data.personal.phone} <Phone size={14} />
                            </a>
                        )}
                        {data.personal.location && <div className="flex items-center gap-2">{data.personal.location} <MapPin size={14} /></div>}
                            ...
                            {data.experience.map((exp) => (
                                <div key={exp.id} id={`item-${exp.id}`} className="mb-4">
                                    ...
                                    <div className="flex justify-between items-center mb-2">
                                        <span className="text-sm font-semibold text-gray-700">{exp.role}</span>
                        ...
                        <div key={sectionId} id={`section-${sectionId}`} className="mb-6 break-inside-avoid">
                            ...
                            <div className="flex flex-wrap gap-2 w-full">
                                {data.skills.map((skill, index) => (
                                    <span key={index} className={`text-xs px-2 py-1 rounded bg-gray-100 font-medium break-inside-avoid`}>
                                        {skill}
                                    </span>
                                ))}
                            </div>
