...
const LayoutModernGrid = ({ data, theme, pageIndex, isMeasurement }) => {
    return (
        <div className={`w-full flex flex-col bg-white ${isMeasurement ? 'h-auto overflow-visible' : 'h-[297mm] overflow-hidden'} ${theme.font}`}>
            {pageIndex === 0 && (
                <div id="section-personal" className={`${theme.primary} p-10 text-white flex justify-between items-end`}>
                    <div>
            ...
            <div className="flex-1 p-10 flex gap-8">
                ...
                <div className="space-y-8 flex-1 min-w-0">
                    {data.sectionOrder?.filter(id => !['education', 'skills'].includes(id)).map(sectionId => {
                        if (sectionId === 'summary' && data.personal.summary) {
                            return (
                                <div key="summary" id="section-summary" className={`${theme.accent} p-6 rounded-lg`}>
                            ...
                            return (
                                <div key="experience" id="section-experience">
                                    {isFirstPageOfSection && (
                                        <h3 id="section-title-experience" className={`font-bold text-lg mb-4 border-b-2 ${theme.border} pb-2`}>Experience</h3>
                                    )}
                                    {data.experience.map((exp) => (
                                        <div key={exp.id} id={`item-${exp.id}`} className="mb-5 last:mb-0">
                        ...
                        if (customSection && customSection.items.length > 0) {
                            return (
                                <div key={sectionId} id={`section-${sectionId}`}>
                                    <h3 className={`font-bold text-lg mb-4 border-b-2 ${theme.border} pb-2`}>{customSection.title}</h3>
                                    {customSection.items.map((item) => (
                                        <div key={item.id} id={`item-${item.id}`} className="mb-5 last:mb-0">
                                            <div className="break-inside-avoid">
                                            ...
                                            {item.description && <p className="text-xs text-gray-600 leading-relaxed whitespace-pre-line">{item.description}</p>}
                                        </div>
                                    ))}
                                </div>
                            );
                        ...
                        if (sectionId === 'education' && data.education.length > 0) {
                            ...
                            return (
                                <div key="education" id="section-education">
                                    {isFirstPageOfSection && (
                                        <h3 id="section-title-education" className={`font-bold text-lg mb-4 border-b-2 ${theme.border} pb-2`}>Education</h3>
                                    )}
                                    {data.education.map((edu) => (
                                        <div key={edu.id} id={`item-${edu.id}`} className="mb-4 break-inside-avoid">
                        ...
                        if (sectionId === 'skills' && data.skills.length > 0) {
                            return (
                                <div key="skills" id="section-skills">
                                    <h3 className={`font-bold text-lg mb-4 border-b-2 ${theme.border} pb-2`}>Skills</h3>
                                    <div className="flex flex-wrap gap-2">
                                        {data.skills.map((skill, i) => (
                                            <span key={i} className={`text-xs font-semibold px-3 py-1 border ${theme.border} rounded-full ${theme.text}`}>
                                                {skill}
                                            </span>
                                        ))}
                                    </div>
