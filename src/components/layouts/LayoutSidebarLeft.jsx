        ...
        if (sectionId === 'summary') {
            return (
                <div key="summary" id={`section-summary`} className="mb-8">
                    <h3 className={`text-lg font-bold uppercase tracking-widest mb-4 ${theme.text}`}>Profile</h3>
            ...
            return (
                <div key="experience" id="section-experience" className="mb-8">
                    {isFirstPageOfSection && (
                        <h3 id="section-title-experience" className={`text-lg font-bold uppercase tracking-widest mb-6 ${theme.text}`}>Experience</h3>
                    )}
                    {data.experience.map((exp) => (
                        <div key={exp.id} id={`item-${exp.id}`} className="mb-6 border-l-2 border-gray-200 pl-4 relative">
            ...
            <div className={`w-1/3 ${theme.primary} text-white p-[15mm] flex flex-col min-h-full`}>
                {pageIndex === 0 && (
                    <div id="section-personal">
                        ...
                        <div className="mb-8 space-y-3 text-white/90">
                            <div className="flex items-center gap-2 text-xs"><Mail size={14} /> {data.personal.email}</div>
                            <div className="flex items-center gap-2 text-xs"><Phone size={14} /> {data.personal.phone}</div>
                            <div className="flex items-center gap-2 text-xs"><MapPin size={14} /> {data.personal.location}</div>
                            {(data.personal.socials || []).map(social => (
                                <div key={social.id} className="flex items-center gap-2 text-xs">
                ...
                {data.education && data.education.length > 0 && (
                    <div id="section-education" className="mb-8">
                        <h3 id="section-title-education" className="text-xs uppercase tracking-widest font-bold border-b border-white/30 pb-2 mb-3">Education</h3>
