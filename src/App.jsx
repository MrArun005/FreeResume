...
import {
  Layout, ChevronRight, Plus, Trash2,
  Upload, FileText, ArrowLeft, Loader,
  CheckCircle, Printer, Download, LayoutGrid, User, X, SplitSquareHorizontal,
  Undo, Redo, Sparkles, Briefcase, ChevronDown, Bot, Flame
} from 'lucide-react';
...
const App = () => {
    ...
    setResume(prev => ({
      ...
      personal: {
        ...
        socials: [
          ...
          { id: Date.now(), network: 'New Link', url: '' }
        ]
      }
    }));
  };
  ...
  const handleArrayChange = (section, id, field, value) => {
    setResume(prev => ({ ...prev, [section]: prev[section].map(item => item.id === id ? { ...item, [field]: value } : item) }));
  };
  ...
  const addItem = (section) => {
    const newItem = section === 'experience'
      ? { id: Date.now(), role: 'Role', company: 'Company', date: 'Date', description: 'Description' }
    ...
    <div className="min-h-screen bg-transparent text-gray-800 dark:text-gray-100 font-sans flex flex-col h-screen transition-colors duration-300 overflow-x-hidden relative">
      ...
      {isOverflowing && (
        <div className="bg-red-600 text-white px-6 py-3 flex items-center justify-between no-print z-30">
          <div className="flex items-center gap-2">
            ...
            <span className="font-medium">⚠️ Resume exceeds 1 page! Check the gray page separators.</span>
          </div>
        </div>
      )}
                ...
                <div className="p-2 space-y-1">
                  ...
                  >
                    ...
                    <div>
                      ...
                      <div className="text-xs text-gray-500">Rewrite for a specific role</div>
                    </div>
                  </button>

                  <button
                    onClick={() => { handleImproveResume(); setShowAiMenu(false); }}
                    disabled={isImproving}
                  ...
                  >
                    <div className="p-2 bg-emerald-100 text-emerald-600 rounded-lg">
                      {isImproving ? <Loader size={18} className="animate-spin" /> : <Sparkles size={18} />}
                    </div>
                    <div>
                      <div className="font-bold text-sm">Improve Writing</div>
                      <div className="text-xs text-gray-500">Fix grammar & polish tone</div>
                    </div>
                  </button>
          ...
          <div className="relative">
            ...
            {showExportMenu && (
              <div className="absolute top-full right-0 mt-2 w-56 bg-white rounded-xl shadow-2xl border border-gray-100 overflow-hidden z-50 animate-in fade-in slide-in-from-top-2">
                <div className="p-2 space-y-1">
                  <button
                    onClick={() => { setIsRoastModalOpen(true); setShowExportMenu(false); }}
      ...
      <div className="w-full flex-1 flex flex-row overflow-hidden relative pb-16 lg:pb-0">
        ...
        <div className={`${mobileView === 'preview' ? 'flex' : 'hidden'} lg:flex flex-1 bg-gray-200 overflow-auto h-full relative print-area flex-col items-center p-4 lg:p-12 gap-8 pb-24 lg:pb-12 transition-all duration-300 ${isSidebarOpen ? 'xl:pr-[340px]' : 'xl:pr-12'}`}>
          ...
          <div className={`fixed right-0 top-16 bottom-0 w-80 bg-white/80 backdrop-blur-md border-l border-gray-200 p-6 hidden xl:flex flex-col gap-8 no-print z-30 shadow-xl overflow-y-auto transition-transform duration-300 ${isSidebarOpen ? 'translate-x-0' : 'translate-x-full'}`}>
            <div className="flex items-center justify-between">
              <h3 className="font-bold text-gray-900 text-lg uppercase tracking-wider">Templates</h3>
              <div className="flex items-center gap-3">
                <span className="text-xs text-gray-500 font-medium">{TEMPLATES.length} designs</span>
                <button onClick={() => setIsSidebarOpen(false)} className="text-gray-400 hover:text-gray-600 hover:bg-gray-100 p-1 rounded-lg transition">
                  <X size={20} />
                </button>
              </div>
            </div>
          ...
          <div id="measurement-container" style={{ position: 'absolute', visibility: 'hidden', pointerEvents: 'none', width: '210mm', zIndex: -1000 }}>
          ...
          <div className="w-full flex-col items-center print:block print:w-auto" ref={contentRef}>
            {pagedData.map((pageData, index) => (
              <div key={index} style={{ width: `${210 * scale}mm`, height: `${297 * scale}mm`, marginBottom: '2rem' }} className="relative transition-all duration-300 mx-auto print:mx-0 print:mb-0">
                <div
